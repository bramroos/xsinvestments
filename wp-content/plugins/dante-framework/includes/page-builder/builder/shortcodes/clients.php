...
class SwiftPageBuilderShortcode_clients extends SwiftPageBuilderShortcode {
    ...
    protected function content($atts, $content = null) {
			...
			while ( $clients_items->have_posts() ) : $clients_items->the_post();
				...
				$client_link_url = dante_framework_get_post_meta($post->ID, 'sf_client_link', true);
				$target = "_blank";
                $client_link_same_window = dante_framework_get_post_meta($post->ID, 'sf_client_link_same_window', true);
				$image_alt = esc_attr( dante_framework_get_post_meta($client_image, '_wp_attachment_image_alt', true) );
				    				
				$items .= '<div class="clearfix carousel-item client-item '.$item_size_class.'">';
				    				
				$items .= '<figure>';
					
				$image = dante_aq_resize( $client_img_url, $client_width, $client_height, true, false);
				...
				if ($image) {
					...
					$items .= '<a href="'.esc_url( $client_link_url ).'" target="'.$target.'"><img src="'.$image[0].'" width="'.$image[1].'" height="'.$image[2].'" alt="'.$image_alt.'" /></a>';
					} else {
...
SPBMap::map( 'clients', array(
    ...
    "params"	=> array(
        ...
        array(
            ...
            "param_name" => "category",
            "value" => dante_framework_get_category_list('clients-category'),
