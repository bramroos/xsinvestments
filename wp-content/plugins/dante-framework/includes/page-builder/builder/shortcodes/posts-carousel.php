...
class SwiftPageBuilderShortcode_posts_carousel extends SwiftPageBuilderShortcode {
    ...
    protected function content($atts, $content = null) {
			...
			while ( $blog_items->have_posts() ) : $blog_items->the_post();
				...
				$thumb_type = dante_framework_get_post_meta($post->ID, 'sf_thumbnail_type', true);
				$thumb_image = rwmb_meta('sf_thumbnail_image', 'type=image&size=full');
				$thumb_video = dante_framework_get_post_meta($post->ID, 'sf_thumbnail_video_url', true);
				$thumb_gallery = rwmb_meta( 'sf_thumbnail_gallery', 'type=image&size=thumb-image' );
				$thumb_link_type = dante_framework_get_post_meta($post->ID, 'sf_thumbnail_link_type', true);
				$thumb_link_url = dante_framework_get_post_meta($post->ID, 'sf_thumbnail_link_url', true);
				$thumb_lightbox_thumb = rwmb_meta( 'sf_thumbnail_image', 'type=image&size=large' );
				$thumb_lightbox_image = rwmb_meta( 'sf_thumbnail_link_image', 'type=image&size=large' );
				$thumb_lightbox_video_url = dante_framework_get_post_meta($post->ID, 'sf_thumbnail_link_video_url', true);
				$thumb_lightbox_video_url = spb_get_embed_src($thumb_lightbox_video_url);
				...
				if ($thumb_type == "video") {
						...
						$video = dante_video_embed($thumb_video, 270, 202);
				...
				if ($show_details == "yes") {
					$items .= '<div class="post-details">'. sprintf(__('%1$s - %2$s', 'swiftframework'), $post_date, dante_get_comments_number($post->ID)) .'</div>';
				}
...
SPBMap::map( 'posts_carousel', array(
    ...
    "params"	=> array(
        ...
        array(
            ...
            "param_name" => "category",
            "value" => dante_framework_get_category_list('category'),
