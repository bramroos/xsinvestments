	...
	*/
	...
	class SwiftPageBuilderShortcode_spb_tabs extends SwiftPageBuilderShortcode {
	    ...
	    public function contentAdmin($atts, $content = null) {
			...
			$exclude_codes = 'spb_tab'; 
			$tab_content = preg_replace("~(?:\[/?)(?!(?:$exclude_codes))[^/\]]+/?\]~s", '', $content);
            preg_match_all( '/" id="([^"]+)"/', $tab_content, $matches, PREG_OFFSET_CAPTURE );
	    ...
	    public function content($atts, $content =null) {
            ...
            foreach ( $tab_titles as $tab ) {
                ...
                if ( $tab_count == 0 ) {
                    $tabs_nav .= '<li class="active"><a href="#' . preg_replace("#[[:punct:]]#", "", (strtolower(str_replace(' ', '-', $tab_id)))) . '" data-toggle="tab"><span>' . $icon_text . $tab[0] . '</span></a></li>';      
                } else {
                    $tabs_nav .= '<li><a href="#' . preg_replace("#[[:punct:]]#", "", (strtolower(str_replace(' ', '-', $tab_id)))) . '" data-toggle="tab"><span>' . $icon_text . $tab[0] . '</span></a></li>';                    
                }
