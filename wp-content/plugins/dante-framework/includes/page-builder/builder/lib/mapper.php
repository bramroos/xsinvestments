	...
	class SPBMap {
	    ...
	    public static function map( $name, $attributes ) {
	        if( empty($attributes['name']) ) {
	            trigger_error( __("Wrong name for shortcode:" . $name, 'dante-framework'));
	        } elseif( empty($attributes['base']) ) {
	            trigger_error( __("Wrong base for shortcode:" . $name, 'dante-framework'));
	        } else {
	    ...
	    public static function addParam($name, $attribute = Array()) {
	        if( !isset(self::$sc[$name]))
	            return trigger_error( __("Wrong name for shortcode:" . $name, 'dante-framework'));
	        elseif (!isset($attribute['param_name'])) {
	            trigger_error( __("Wrong attribute for '" . $name, 'dante-framework'));
	        } else {
