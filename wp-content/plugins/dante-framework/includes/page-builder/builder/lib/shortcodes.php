	...
	*/
	...
	abstract class SwiftPageBuilderShortcode extends SFPageBuilderAbstract {
	            ...
	            foreach ( $this->settings['params'] as $param ) {
	                if ( $param['param_name'] != 'content' ) {
	                    ...
	                    if ( isset($param['value']) ) {
	                        $shortcode_attributes[$param['param_name']] = is_string($param['value']) ? __($param['value'], 'dante-framework') : $param['value'];
	                    } else {
	                        $shortcode_attributes[$param['param_name']] = '';
	                    }
	                } else if ( $param['param_name'] == 'content' && $content == NULL ) {
	                    $content = __($param['value'], 'dante-framework');
	                }
	            }
        ...
        public function startRow( $position, $col_width = "", $fullwidth = false, $content_mode = "", $alt_style = "", $id = "" ) {
            ...
            if ( strpos( $position, 'first' ) !== false ) {
                ...
                } else if ( $dante_sidebar_config == "no-sidebars" ) {
	...
	---------------------------------------------------------- */
	    public function getColumnControls($controls) {
	        ...
	        $controls_column_size = ' <div class="column_size_wrapper"> <a class="column_decrease" href="#" title="'.__('Decrease width', 'dante-framework').'"></a> <span class="column_size">%column_size%</span> <a class="column_increase" href="#" title="'.__('Increase width', 'dante-framework').'"></a> </div>';
	
	        $controls_edit = ' <a class="column_edit" href="#" title="'.__('Edit', 'dante-framework').'"></a>';
	        $controls_popup = ' <a class="column_popup" href="#" title="'.__('Pop up', 'dante-framework').'"></a>';
	        $controls_delete = ' <a class="column_clone" href="#" title="'.__('Clone', 'dante-framework').'"></a> <a class="column_delete" href="#" title="'.__('Delete', 'dante-framework').'"></a>';
	        // $delete_edit_row = '<a class="row_delete" title="'.__('Delete %element%', 'dante-framework').'">'.__('Delete %element%', 'dante-framework').'</a>';
	   ...
	   ---------------------------------------------------------- */
	    ...
	    public function singleParamHtmlHolder($param, $value) {
	        ...
	        $value = str_ireplace($old_names, $new_names, $value);
	            //$value = __($value, 'dante-framework');
	            //
	...
	abstract class SwiftPageBuilderShortcode_UniversalAdmin extends SwiftPageBuilderShortcode {
	    ...
	    public function contentAdmin($atts,  $content) {
	        ...
	        if ( isset($this->settings['params']) ) {
	            ...
	            //$output .= '<div class="span12 spb_edit_form_elements"><h2>'.__('Edit', 'dante-framework').' ' .__($this->settings['name'], 'dante-framework').'</h2>';
				$output .= '<div class="spb_edit_form_elements">';
				
				$output .= '<div id="edit-modal-header">';
				$output .= '<h2>'.__('Edit', 'dante-framework').' ' .__($this->settings['name'], 'dante-framework').'</h2>';
				$output .= '<div class="edit_form_actions"><a href="#" class="spb_save_edit_form button-primary">'. __('Save', 'dante-framework') .'</a></div>';
				$output .= '</div>';
	    ...
	    protected function singleParamEditHolder( $param, $param_value ) {
            ...
            if ( $param['type'] == "section" ) {
                ...
                $output .= '<div class="span12 spb_element_section">' . __( $param['heading'], 'dante-framework' ) . '</div>';

            } else {

                $output .= '<label class="spb_element_label">' . __( $param['heading'], 'dante-framework' ) . '</label>';

                $output .= '<div class="edit_form_line">';
                $output .= $this->singleParamEditForm( $param, $param_value );
                $output .= ( isset( $param['description'] ) ) ? '<span class="description">' . __( $param['description'], 'dante-framework' ) . '</span>' : '';
                $output .= '</div>';
	    ...
	    protected function singleParamEditForm( $param, $param_value ) {
            ...
            if ( $param['type'] == 'textfield' ) {
                $value = __( $param_value, 'dante-framework' );
                $value = $param_value;

                if ( $param['param_name'] == 'address' && $param_value == __( 'Click the edit button to change the map pin details.', 'dante-framework' ) ) {
                    $param_line .= '<input name="' . $param['param_name'] . '" class="spb_param_value spb-textinput ' . $param['param_name'] . ' ' . $param['type'] . '" type="text" value="" data-previous-text="' . __( 'Click the edit button to change the map pin details.', 'dante-framework' ) . '"/>';
                } else {
	            ...
	            foreach ( $param['value'] as $text_val => $val ) {
	                ...
	                $text_val = __( $text_val, 'dante-framework' );
	                $checked = '';
	                if ( $val == $param_value ) {
                ...
                // $param_value = __($param_value, 'dante-framework');
                ...
                $param_line .= '<a class="button gallery_widget_add_images" href="#" title="' . __( 'Add images', 'dante-framework' ) . '">' . __( 'Add images', 'dante-framework' ) . '</a>';
                ...
                $param_line .= '<input type="hidden" class="spb_param_value gallery_widget_attached_images_ids ' . $param['param_name'] . ' ' . $param['type'] . '" name="' . $param['param_name'] . '" value="' . $param_value . '" />';
                $param_line .= '<a class="button gallery_widget_add_images" href="#" use-single="true" title="' . __( 'Add image', 'dante-framework' ) . '" data-uploader_title="' . __( 'Add image', 'dante-framework' ) . '">' . __( 'Add image', 'dante-framework' ) . '</a>';
                $param_line .= '<div class="gallery_widget_attached_images">';
                $param_line .= '<ul class="gallery_widget_attached_images_list">';
                $param_line .= ( $param_value != '' ) ? spb_fieldAttachedImages( explode( ",", $param_value ) ) : '';
                $param_line .= '</ul>';
                ...
                foreach ( $sidebars as $sidebar ) {
                    ...
                    $sidebar_name = __( $sidebar["name"], 'dante-framework' );
	    ...
	    protected function getTinyHtmlTextArea($param = array(), $param_value) {
	        ...
	        if ( function_exists('wp_editor') ) {
	            $default_content = __($param_value, 'dante-framework');
	            $output_value = '';
	            // WP 3.3+
	            ob_start();
	...
	class SwiftPageBuilderShortcode_Settings extends SwiftPageBuilderShortcode_UniversalAdmin {
	    ...
	    public function contentAdmin($atts, $content) {
	        ...
	        if ( isset($this->settings['params']) ) {
				...
				$output .= '<div id="edit-modal-header">';
				$output .= '<h2>'.__('Edit', 'dante-framework').' ' .__($this->settings['name'], 'dante-framework').'</h2>';
				$output .= '<div class="edit_form_actions"><a href="#" id="cancel-background-options">' . __('Cancel', 'dante-framework') . '</a><a href="#" class="spb_save_edit_form button-primary">'. __('Save', 'dante-framework') .'</a></div>';
				$output .= '</div>';
