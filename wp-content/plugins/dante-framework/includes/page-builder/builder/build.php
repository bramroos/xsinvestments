	...
	*/
	...
	class SFPageBuilderSetupAdmin extends SFPageBuilderSetup {
	    ...
	    public function swiftPageBuilderScripts() {
			...
			wp_enqueue_style( 'font-awesome-spb' );
		    wp_enqueue_style( 'ssgizmo-spb' );
	    ...
	    public function registerCss() {
	        ...
	        wp_register_style( 'font-awesome-spb',  get_template_directory_uri() . '/css/font-awesome.min.css', array(), '5.10.1', 'all' );
	        wp_register_style( 'ssgizmo-spb', get_template_directory_uri() . '/css/ss-gizmo.css', array(), null, 'all' );
	    }
	    
	    public function swiftPageBuilderEditPage() {
	        $pt_array = $this->swift_page_builder->getPostTypes();
	        foreach ($pt_array as $pt) {
	            add_meta_box( 'swift_page_builder', __('Swift Page Builder', 'dante-framework'), Array($this->swift_page_builder->getLayout(), 'output'), $pt, 'normal', 'high');
	        }
	    }
