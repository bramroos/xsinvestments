	...
	*/
	...
	class sf_infocus extends WP_Widget {
		...
		function widget( $args, $instance ) {
			...
			<div class="infocus-item">
				...
				<?php 
					...
					$thumb_video = dante_framework_get_post_meta($post_id, 'sf_thumbnail_video_url', true);
					...
					$image = dante_aq_resize( $thumb_img_url, 300, 225, true, false);
				?>
				<figure class="animated-overlay overlay-alt">
					<?php if ($thumb_video != "") { ?>
						<?php echo dante_video_embed($thumb_video, 300, 200); ?>
					<?php } else if ($image) { ?>
		...
		function form( $instance ) {	
				...
				<p>
					<label for="<?php echo $this->get_field_id('post_id'); ?>"><?php esc_html_e( 'Post ID:', 'dante-framework' ); ?></label>
