<?php
	...
	*
	*	dante_get_post_media()
	*	dante_get_post_format_image_src()
	*	dante_image_post()
	*	dante_video_post()
	*	dante_gallery_post()
	*	dante_audio_post()
	...
	*/
	...
	================================================== */
	if (!function_exists('dante_get_post_media')) {
		function dante_get_post_media($postID, $media_width, $media_height, $video_height, $use_thumb_content) {
			
			$format = get_post_format($postID);
			$post_media = "";
			
			if ($format == "image") {
				$post_media = dante_image_post($postID, $media_width, $media_height, $use_thumb_content);
			} else if ($format == "video") {
				$post_media = dante_video_post($postID, $media_width, $video_height, $use_thumb_content);
			} else if ($format == "gallery") {
				$post_media = dante_gallery_post($postID, $use_thumb_content);
			} else if ($format == "audio") {
				$post_media = dante_audio_post($postID);
			} else if ($format == "link") {
				$post_media = dante_link_post($postID);
			} else if ($format == "chat") {
				$post_media = dante_chat_post($postID);
			}
	...
	/* GET IMAGE MEDIA
	================================================== */
	if (!function_exists('dante_get_post_format_image_src')) {
		function dante_get_post_format_image_src($post_id){
	...
	if (!function_exists('dante_image_post')) {
		function dante_image_post($postID, $media_width, $media_height, $use_thumb_content) {
			...
			$detail_image = dante_aq_resize( $media_image_url, $media_width, $media_height, true, false);
			$image_alt = dante_get_post_meta($image_id, '_wp_attachment_image_alt', true);
	...
	/* GET VIDEO MEDIA
	================================================== */ 
	if (!function_exists('dante_video_post')) {
		function dante_video_post($postID, $media_width, $video_height, $use_thumb_content) {
			
			$video = $media_video = "";
						
			if ($use_thumb_content) {
			$media_video = dante_get_post_meta($postID, 'sf_thumbnail_video_url', true);
			} else {
			$media_video = dante_get_post_meta($postID, 'sf_detail_video_url', true);
			}
			
			$video = dante_video_embed($media_video, $media_width, $video_height);
	...
	/* GET GALLERY MEDIA
	================================================== */ 
	if (!function_exists('dante_gallery_post')) {
		function dante_gallery_post($postID, $use_thumb_content) {
	...
	/* GET AUDIO MEDIA
	================================================== */ 
	if (!function_exists('dante_audio_post')) {
		function dante_audio_post($postID) {
			$audio = "";
	...
	/* GET LINK MEDIA
	================================================== */ 
	if (!function_exists('dante_link_post')) {
		function dante_link_post($postID) {
	...
	/* GET CHAT MEDIA
	================================================== */ 
	if (!function_exists('dante_chat_post')) {
		function dante_chat_post($postID) {
	...
	/* GET POST ITEM
	================================================== */ 
	if (!function_exists('dante_get_post_item')) {
		function dante_get_post_item($postID, $blog_type, $show_title = "yes", $show_excerpt = "yes", $show_details = "yes", $excerpt_length = "20", $content_output = "excerpt", $show_read_more = "no") {
			...
			global $post, $dante_sidebar_config;
			$post_id = $post->ID;
			...
			$custom_excerpt = dante_get_post_meta($post_id, 'sf_custom_excerpt', true);
			$post_excerpt = '';
			if ($content_output == "excerpt") {
				if ($custom_excerpt != '') {
				$post_excerpt = dante_custom_excerpt($custom_excerpt, $excerpt_length);
				} else {
				if ($post_format == "quote") {
				$post_excerpt = dante_get_the_content_with_formatting();
				} else {
				$post_excerpt = dante_excerpt($excerpt_length);
				}
				}
			} else {
				$post_excerpt = dante_get_the_content_with_formatting();
			...
			if ($blog_type == "mini") {
				...
				if ($dante_sidebar_config == "both-sidebars" || $blog_type == "masonry-fw") {
				$item_class = "col-sm-3";
				} else {
				$item_class = "col-sm-4";
				}
			...
			$thumb_type = dante_get_post_meta($post_id, 'sf_thumbnail_type', true);
			$thumb_image = rwmb_meta('sf_thumbnail_image', 'type=image&size=full');
			$thumb_video = dante_get_post_meta($post_id, 'sf_thumbnail_video_url', true);
			$thumb_gallery = rwmb_meta( 'sf_thumbnail_gallery', 'type=image&size='.$gallery_size);
			$thumb_link_type = dante_get_post_meta($post_id, 'sf_thumbnail_link_type', true);
			$thumb_link_url = dante_get_post_meta($post_id, 'sf_thumbnail_link_url', true);
			$thumb_lightbox_thumb = rwmb_meta( 'sf_thumbnail_image', 'type=image&size=large' );
			$thumb_lightbox_image = rwmb_meta( 'sf_thumbnail_link_image', 'type=image&size=large' );
			$thumb_lightbox_video_url = dante_get_post_meta($post_id, 'sf_thumbnail_link_video_url', true);
			$thumb_lightbox_video_url = dante_get_embed_src($thumb_lightbox_video_url);
			$image_id = 0;
			...
			if ($thumb_type == "video") {
				...
				$video = dante_video_embed($thumb_video, $thumb_width, $video_height);
			...
			if ($blog_type == "masonry" || $blog_type == "masonry-fw") {
				...
				if ($show_details == "yes" && $post_format != "quote" && $post_format != "link") {
					...
					if ($single_author && !$remove_dates) {
						...
						$post_item .= '<div class="blog-item-details vcard author">'. sprintf(__('By <span itemprop="author" class="fn">%1$s</span> on <span class="date updated">%2$s</span>', 'dante'), $post_author, $post_date) .'</div>';
					} else if (!$single_author) {
						$post_item .= '<div class="blog-item-details vcard author">'. sprintf(__('By <span itemprop="author" class="fn">%1$s</span>', 'dante'), $post_author) .'</div>';
					}
				}
				...
				if ($show_read_more == "yes") {
					$post_item .= '<a class="read-more-button" href="'.$post_permalink.'">'.__("Read more", 'dante').'</a>';
				}
			...
			// STANDARD STYLING
				...
				if ($show_details == "yes") {
					...
					if ($post_format == "quote" || $post_format == "link") {
						if ($single_author && !$remove_dates) {
							$post_item .= '<div class="blog-item-details">'. sprintf(__('%1$s', 'dante'), $post_date) .'</div>';
						} else if (!$remove_dates) {
							$post_item .= '<div class="blog-item-details">'. sprintf(__('By <a href="%2$s" rel="author" itemprop="author">%1$s</a> on %3$s', 'dante'), $post_author, get_author_posts_url(get_the_author_meta( 'ID' )), $post_date) .'</div>';
						} else if (!$single_author) {
							$post_item .= '<div class="blog-item-details">'. sprintf(__('By <a href="%2$s" rel="author" itemprop="author">%1$s</a>', 'dante'), $post_author, get_author_posts_url(get_the_author_meta( 'ID' ))) .'</div>';
						}
					}
	...
	/* GET SEARCH ITEM
	================================================== */ 
	if (!function_exists('dante_get_search_item')) {
		function dante_get_search_item($postID) {
			...
			$image = dante_aq_resize( $thumb_img_url, 70, 70, true, false);
			$image_title = dante_featured_img_title();
