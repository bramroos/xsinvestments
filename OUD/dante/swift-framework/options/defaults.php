    ...
    class Redux_Options {
        ...
        */
        function __construct($sections = array(), $args = array(), $extra_tabs = array()) {    
            ...
            $defaults['last_tab'] = '0';
            $defaults['menu_icon'] = "";
            $defaults['menu_title'] = __('Options', 'dante');
            $defaults['page_icon'] = 'fa-themes';
            $defaults['page_title'] = __('Options', 'dante');
        ...
        */
        function _enqueue() {
			...
			wp_register_style(
				'redux-font-awesome',
				DANTE_LOCAL_PATH . '/css/font-awesome.min.css',
				array(),
				time(),
				'all'
			);
...
//			);
            ...
            if($this->args['admin_stylesheet'] == 'custom') {
            ...
            wp_localize_script('redux-opts-js', 'redux_opts', array('reset_confirm' => __('Are you sure? You will lose all custom values.', 'dante'), 'opt_name' => $this->args['opt_name']));
        ...
        */
        function _options_page_html() {
            ...
            // WELCOME MESSAGE
//            if (isset($_GET['dante_welcome'])) {
//            	if ($_GET['dante_welcome'] == true) {
//            		echo '<div id="sf-welcome"></div>';	
//            	}
//            }
            ...
            echo ' &nbsp; ';
            submit_button(__('Reset to Defaults', 'dante'), 'secondary', $this->args['opt_name'] . '[defaults]', false);
            ...
            if(isset($_GET['settings-updated']) && $_GET['settings-updated'] == 'true' && get_transient('redux-opts-saved') == '1') {
                if(isset($this->options['imported']) && $this->options['imported'] == 1) {
                    echo '<div id="redux-opts-imported">' . apply_filters('redux-opts-imported-text-' . $this->args['opt_name'], __('<strong>Settings Imported!</strong>', 'dante')) . '</div>';
                } else {
                    echo '<div id="redux-opts-save">' . apply_filters('redux-opts-saved-text-' . $this->args['opt_name'], __('<strong>Settings Saved!</strong>', 'dante')).'</div>';
                }
                delete_transient('redux-opts-saved');
            }

            echo '<div id="redux-opts-save-warn">' . apply_filters('redux-opts-changed-text-' . $this->args['opt_name'], __('<strong>Settings have changed, you should save them!</strong>', 'dante')) . '</div>';
            echo '<div id="redux-opts-field-errors">' . __('<strong><span></span> error(s) were found!</strong>', 'dante') . '</div>';
            echo '<div id="redux-opts-field-warnings">' . __('<strong><span></span> warning(s) were found!</strong>', 'dante').'</div>';
            ...
            foreach($this->sections as $k => $section) {
				if($this->args['icon_type'] == 'image' || (isset($section['icon_type']) && $section['icon_type'] == 'image')) {
					...
					$icon = (!isset($section['icon'])) ? '<i class="fa-cog' . $icon_class . '"></i> ' : '<i class="fa-' . $section['icon'] . $icon_class . '"></i> ';
				} else {
					$icon_class = (!isset($section['icon_class'])) ? '' : ' ' . $section['icon_class'];
					$icon = (!isset($section['icon'])) ? '<i class="fa-cog' . $icon_class . '"></i> ' : '<i class="fa-' . $section['icon'] . $icon_class . '"></i> ';
				}
            ...
            if(true === $this->args['show_import_export']) {
				...
				if($this->args['import_icon_type'] == 'image') {
					...
					$icon = (!isset($this->args['import_icon'])) ? '<i class="fa-refresh' . $icon_class . '"></i>' : '<i class="fa-' . $this->args['import_icon'] . $icon_class . '"></i> ';
				}
                echo '<a href="javascript:void(0);" id="import_export_default_section_group_li_a" class="redux-opts-group-tab-link-a" data-rel="import_export_default">' . $icon . ' <span>' . __('Import / Export', 'dante') . '</span></a>';
                echo '</li>';
            ...
            if(is_array($this->extra_tabs)) {
				foreach($this->extra_tabs as $k => $tab) {
					if($this->args['icon_type'] == 'image' || (isset($tab['icon_type']) && $tab['icon_type'] == 'image')) {
    	                ...
    	                $icon = (!isset($tab['icon'])) ? '<i class="fa-cog' . $icon_class . '"></i> ' : '<i class="fa-' . $tab['icon'] . $icon_class . '"></i> ';
					} else {
						$icon_class = (!isset($tab['icon_class'])) ? '' : ' ' . $tab['icon_class'];
    	                $icon = (!isset($tab['icon'])) ? '<i class="fa-cog' . $icon_class . '"></i> ' : '<i class="fa-' . $tab['icon'] . $icon_class . '"></i> ';
					}
            ...
            if(true === $this->args['dev_mode']) {
				...
				if($this->args['dev_mode_icon_type'] == 'image') {
					...
					$icon = (!isset($this->args['dev_mode_icon'])) ? '<i class="fa-info-sign' . $icon_class . '"></i>' : '<i class="fa-' . $this->args['dev_mode_icon'] . $icon_class . '"></i> ';
				}
                echo '<a href="javascript:void(0);" id="dev_mode_default_section_group_li_a" class="redux-opts-group-tab-link-a custom-tab" data-rel="dev_mode_default">' . $icon . ' <span>' . __('Dev Mode Info', 'dante') . '</span></a>';
                echo '</li>';
            }
            ...
            if(true === $this->args['show_import_export']) {
                ...
                echo '<h3>' . __('Import / Export Options', 'dante') . '</h3>';
                echo '<h4>' . __('Import Options', 'dante') . '</h4>';
                echo '<p><a href="javascript:void(0);" id="redux-opts-import-code-button" class="button-secondary">' . __('Import from file', 'dante') . '</a> <a href="javascript:void(0);" id="redux-opts-import-link-button" class="button-secondary">' . __('Import from URL', 'dante') . '</a></p>';

                echo '<div id="redux-opts-import-code-wrapper">';

                echo '<div class="redux-opts-section-desc">';
                echo '<p class="description" id="import-code-description">' . apply_filters('redux-opts-import-file-description', __('Input your backup file below and hit Import to restore your sites options from a backup.', 'dante')) . '</p>';
                echo '</div>';
                ...
                echo '<div class="redux-opts-section-desc">';
                echo '<p class="description" id="import-link-description">' . apply_filters('redux-opts-import-link-description', __('Input the URL to another sites options set and hit Import to load the options from that site.', 'dante')) . '</p>';
                echo '</div>';
                ...
                echo '<p id="redux-opts-import-action"><input type="submit" id="redux-opts-import" name="' . $this->args['opt_name'] . '[import]" class="button-primary" value="' . __('Import', 'dante') . '"> <span>' . apply_filters('redux-opts-import-warning', __('WARNING! This will overwrite any existing options, please proceed with caution!', 'dante')) . '</span></p>';
                echo '<div id="import_divide"></div>';

                echo '<h4>' . __('Export Options', 'dante') . '</h4>';
                echo '<div class="redux-opts-section-desc">';
                echo '<p class="description">' . apply_filters('redux-opts-backup-description', __('Here you can copy/download your current option settings. Keep this safe as you can use it as a backup should anything go wrong, or you can use it to restore your settings on this site (or any other site).', 'dante')).'</p>';
                echo '</div>';

                echo '<p><a href="javascript:void(0);" id="redux-opts-export-code-copy" class="button-secondary">' . __('Copy', 'dante') . '</a> <a href="'.esc_url( add_query_arg(array('feed' => 'reduxopts-'.$this->args['opt_name'], 'action' => 'download_options', 'secret' => md5(AUTH_KEY.SECURE_AUTH_KEY)), site_url()) ).'" id="redux-opts-export-code-dl" class="button-primary">Download</a> <a href="javascript:void(0);" id="redux-opts-export-link" class="button-secondary">' . __('Copy Link', 'dante') . '</a></p>';
            ...
            if(true === $this->args['dev_mode']) {
                ...
                echo '<h3>' . __('Dev Mode Info', 'dante') . '</h3>';
                echo '<div class="redux-opts-section-desc">';
