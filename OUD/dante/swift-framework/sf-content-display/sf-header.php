<?php
	...
	*/
 	...
 	/* TOP BAR
 	================================================== */
 	if (!function_exists('dante_top_bar')) {
		function dante_top_bar() {
			...
			if ($tb_config == "tb-1") {
				...
				$tb_left_output = dante_woo_links('top-menu', $tb_config);
				$tb_right_output = dante_aux_links('top-menu');
				...
				$tb_left_output = dante_aux_links('top-menu');
				$tb_right_output = '<div class="tb-text clearfix">'. do_shortcode($tb_right_text). '</div>' . "\n";
	...
	/* HEADER
	================================================== */
	if (!function_exists('dante_header')) {
		function dante_header($header_layout) {

			// VARIABLES
			$options = get_option('sf_dante_options');
			$show_cart = $options['show_cart'];
			$show_wishlist = $options['show_wishlist'];
			$header_left_text = __($options['header_left_text'], 'dante');
			...
			$header_output .= dante_logo('col-sm-4 logo-center');
			$header_output .= '<div class="header-right col-sm-4">'.dante_aux_links('header-menu', TRUE, "header-1").'</div>'. "\n";
			$header_output .= '</div> <!-- CLOSE .row -->'. "\n";
			$header_output .= '</div> <!-- CLOSE .container -->'. "\n";
			$header_output .= '</header>'. "\n";
			$header_output .= '<div id="main-nav" class="sticky-header">'. "\n";
			$header_output .= dante_main_menu('main-navigation', 'full');
			$header_output .= '</div>'. "\n";
			...
			$header_output .= '<div class="container">'. "\n";
			$header_output .= '<div class="row">'. "\n";
			$header_output .= dante_logo('col-sm-4 logo-left');
			$header_output .= '<div class="header-right col-sm-8">'.dante_aux_links('header-menu', FALSE, "header-1").'</div>'. "\n";
			$header_output .= '</div> <!-- CLOSE .row -->'. "\n";
			$header_output .= '</div> <!-- CLOSE .container -->'. "\n";
			$header_output .= '</header>'. "\n";
			$header_output .= '<div id="main-nav" class="sticky-header">'. "\n";
			...
			$header_output .= '<div class="row">'. "\n";
			$header_output .= dante_logo('logo-left');
			$header_output .= '<div class="header-right">';
			$header_output .= dante_main_menu('main-navigation', 'with-search');
			$header_output .= '</div>'. "\n";
			$header_output .= '</div> <!-- CLOSE .row -->'. "\n";
			$header_output .= '</div> <!-- CLOSE .container -->'. "\n";
			$header_output .= '</header>'. "\n";
			...
			$header_output .= '<header id="header" class="clearfix">'. "\n";
			$header_output .= dante_top_header();
			$header_output .= '<div class="container header-container sticky-header">'. "\n";
			$header_output .= '<div class="row">'. "\n";
			$header_output .= dante_logo('logo-left');
			$header_output .= '<div class="header-right">';
			...
			$header_output .= '<div class="row">'. "\n";
			$header_output .= dante_logo('logo-left');
			$header_output .= '<div class="header-right">'.dante_main_menu('main-navigation', 'with-search').'</div>'. "\n";
			$header_output .= '</div> <!-- CLOSE .row -->'. "\n";
			$header_output .= '</div> <!-- CLOSE .container -->'. "\n";
			$header_output .= '</header>'. "\n";
	...
	/* TOP HEADER
	================================================== */
	if (!function_exists('dante_top_header')) {
		function dante_top_header() {
			...
			$top_header_output .= dante_aux_links('top-header-menu');
	...
	/* LOGO
	================================================== */
	if (!function_exists('dante_logo')) {
		function dante_logo($logo_class) {

			//VARIABLES
			global $woocommerce;
			$options = get_option('sf_dante_options');
			$show_cart = $options['show_cart'];
			$logo = $light_logo = $dark_logo = "";
			if (isset($options['logo_upload'])) {
			$logo = __( $options['logo_upload'] , 'dante' );
			}
			// Light Logo
			if (isset($options['light_logo_upload'])) {
			$light_logo = __( $options['light_logo_upload'] , 'dante' );
			}
			if ( $light_logo != "" ) {
			...
			if (isset($options['dark_logo_upload'])) {
				$dark_logo = __( $options['dark_logo_upload'] , 'dante' );
			}
			if ( $dark_logo != "" ) {
			...
			$logo_link_url = apply_filters( 'dante_logo_link_url', home_url() );
	...
	/* MENU
	================================================== */
	if (!function_exists('dante_main_menu')) {
		function dante_main_menu($id, $layout = "") {
			...
			if ($post) {
			$page_menu = dante_get_post_meta($post->ID, 'sf_page_menu', true);
			}
			$main_menu_args = array(
				'echo'            => false,
				'theme_location' => 'main_navigation',
				'walker' => new dante_mega_menu_walker,
				'fallback_cb' => '',
				'menu' => $page_menu
			);
			...
			if(function_exists('wp_nav_menu')) {
				if (has_nav_menu('main_navigation')) {
					...
					$menu_output .= '<div class="no-menu">'.__("Please assign a menu to the Main Menu in Appearance > Menus", 'dante').'</div>';
				}
			}
			$menu_output .= '</nav>'. "\n";
			...
			if ($layout == "full") {
				...
				$menu_full_output .= '<div class="ajax-search-wrap"><div class="ajax-loading"></div><form method="get" class="ajax-search-form" action="'.home_url().'/"><input type="text" placeholder="'.__("Search", 'dante').'" name="s" autocomplete="off" /></form><div class="ajax-search-results"></div></div>'. "\n";
				$menu_full_output .= '</li>'. "\n";
				}
				if ($show_cart) {
				...
				$menu_with_search_output .= '<div class="ajax-search-wrap"><div class="ajax-loading"></div><form method="get" class="ajax-search-form" action="'.home_url().'/"><input type="text" placeholder="'.__("Search", 'dante').'" name="s" autocomplete="off" /></form><div class="ajax-search-results"></div></div>'. "\n";
	...
	/* MOBILE MENU
	================================================== */
	if (!function_exists('dante_mobile_menu')) {
		function dante_mobile_menu() {
			...
			$mobile_menu_args = array(
				...
				'walker'         => new dante_alt_menu_walker,
				'fallback_cb' => '',
				'menu' => $page_menu
			);
			...
			if ($show_translation) {
			$mobile_menu_output .= '<ul class="mobile-language-select">'.dante_language_flags().'</ul>'. "\n";
			}
			$mobile_menu_output .= '<form method="get" class="mobile-search-form" action="'.home_url().'/"><input type="text" placeholder="'.__("Search", 'dante').'" name="s" autocomplete="off" /></form>'. "\n";
	...
	/* WOO LINKS
	================================================== */
	if (!function_exists('dante_woo_links')) {
		function dante_woo_links($position, $config = "") {
			...
			if (is_user_logged_in()) {
				...
				$woo_links_output .= '<li class="tb-welcome">' . __("Welcome", 'dante') . " " . $current_user->display_name . '</li>'. "\n";
			} else {
				$woo_links_output .= '<li class="tb-welcome">' . __("Welcome", 'dante') . '</li>'. "\n";
			}
			if ($ss_enable) {
	...
	/* AUX LINKS
	================================================== */
	if (!function_exists('dante_aux_links')) {
		function dante_aux_links($position, $alt_version = FALSE, $header_version = "") {
			...
			$my_account_link = apply_filters('dante_header_myaccount_url', $my_account_link);
			$options = get_option('sf_dante_options');
			$show_sub = $options['show_sub'];
			$show_translation = $options['show_translation'];
			...
			$sub_text = apply_filters('dante_subscribe_text', __("Subscribe", 'dante'));
			...
			if ($show_account) {
				if (is_user_logged_in()) {
					$aux_links_output .= '<li><a href="'.wp_logout_url(home_url()).'">'. __("Sign Out", 'dante') .'</a></li>'. "\n";
					if ( $myaccount_page_id ) {
					$aux_links_output .= '<li><a href="'.$my_account_link.'" class="admin-link">'. __("My Account", 'dante') .'</a></li>'. "\n";
					} else {
					$aux_links_output .= '<li><a href="'.get_admin_url().'" class="admin-link">'. __("My Account", 'dante') .'</a></li>'. "\n";
					}
				} else {
					$aux_links_output .= '<li><a href="'.$login_url.'">'. __("Login", 'dante') .'</a></li>'. "\n";
				}
			}
			if ($show_sub) {
			...
			if ($show_translation) {
				$aux_links_output .= '<li class="parent aux-languages"><a href="#">'. __("Language", 'dante') .'</a>'. "\n";
				$aux_links_output .= '<ul id="header-languages" class="sub-menu">'. "\n";
				if (function_exists( 'dante_language_flags' )) {
				$aux_links_output .= dante_language_flags();
				}
				$aux_links_output .= '</ul>'. "\n";
				$aux_links_output .= '</li>'. "\n";
			}
			if ($header_version != "header-1") {
				if ($show_cart) {
				$aux_links_output .= dante_get_cart();
				}
				if ( class_exists( 'YITH_WCWL_UI' ) && $show_wishlist)  {
				$aux_links_output .= dante_get_wishlist();
				}
			}
	...
	/* CART DROPDOWN
	================================================== */
	if (!function_exists('dante_get_cart')) {
		function dante_get_cart() {
			...
			if (dante_woocommerce_activated()) {
				...
				$cart_count_text = dante_product_items_text($cart_count);
				$price_display_suffix  = get_option( 'woocommerce_price_display_suffix' );
				
				if ($show_cart_count) {
					$cart_output .= '<li class="parent shopping-bag-item"><a class="cart-contents" href="'.$woocommerce->cart->get_cart_url().'" title="'.__("View your shopping cart", 'dante').'"><i class="ss-cart"></i>'.$cart_total.' ('.$cart_count.')</a>';
				} else {
					$cart_output .= '<li class="parent shopping-bag-item"><a class="cart-contents" href="'.$woocommerce->cart->get_cart_url().'" title="'.__("View your shopping cart", 'dante').'"><i class="ss-cart"></i>'.$cart_total.'</a>';
				}
	            ...
	            if ( $cart_count != "0" ) {
	            	...
	            	$cart_output .= '<div class="bag-header">'.$cart_count_text.' '.__('in the shopping bag', 'dante').'</div>';
	            	...
	            	foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
	                    ...
	                    if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
	                        ...
	                        $cart_output .= '<div class="bag-product-price">'.__("Unit Price:", 'dante').'
	                        '.$price.'</div>';
	                        $cart_output .= '<div class="bag-product-quantity">'.__('Quantity:', 'dante').' '.$cart_item['quantity'].'</div>';
	                        if ( $price_display_suffix ) {
	                        	$cart_output .= '<small class="woocommerce-price-suffix">'. $price_display_suffix . '</small>';
	                        }
	                        $cart_output .= '</div>';
	                        $cart_output .= apply_filters( 'woocommerce_cart_item_remove_link', sprintf('<a href="%s" class="remove" title="%s">&times;</a>', esc_url( $woocommerce->cart->get_remove_url( $cart_item_key ) ), __('Remove this item', 'dante') ), $cart_item_key );
	                ...
	                $cart_output .= '<a class="sf-button standard sf-icon-reveal bag-button" href="'.esc_url( $cart_url ).'"><i class="ss-view"></i><span class="text">'. __('View shopping bag', 'dante').'</span></a>';
	               	$cart_output .= '<a class="sf-button standard sf-icon-reveal checkout-button" href="'.esc_url( $checkout_url ).'"><i class="ss-creditcard"></i><span class="text">'.__('Proceed to checkout', 'dante').'</span></a>';

	               	$cart_output .= '</div>';

	            } else {

	           		$cart_output .= '<div class="bag-header">'.__("0 items in the shopping bag", 'dante').'</div>';

	           		$cart_output .= '<div class="bag-empty">'.__('Unfortunately, your shopping bag is empty.','dante').'</div>';
	            	...
	            	$cart_output .= '<a class="sf-button standard sf-icon-reveal shop-button" href="'.esc_url( $shop_page_url ).'"><i class="ss-cart"></i><span class="text">'.__('Go to the shop', 'dante').'</span></a>';
	...
	/* WISHLIST DROPDOWN
	================================================== */
	if (!function_exists('dante_get_wishlist')) {
		function dante_get_wishlist() {
			...
			if ( is_user_logged_in() ) {
			    ...
			    $count = $count[0]['cnt'];
			} else if (is_array($_SESSION['yith_wcwl_products'])) {
			...
			$wishlist_output .= '<li class="parent wishlist-item"><a class="wishlist-link" href="'.$yith_wcwl->get_wishlist_url().'" title="'.__("View your wishlist", 'dante').'"><i class="ss-star"></i><span>'.$count.'</span></a>';
	        ...
	        if ( count( $wishlist ) > 0 ) :
	           	...
	           	foreach( $wishlist as $values ) :
	                ...
	                if ($count_limit < 4) {
		                ...
		                if ( has_post_thumbnail($product_obj->id) ) {
		                	...
		                	$image = dante_aq_resize( $image_link, 70, 70, true, false);
	            ...
	            $wishlist_output .= '<div class="wishlist-empty">'. __( 'Your wishlist is currently empty.', 'dante' ) .'</div>';
	        endif;
			...
			$wishlist_output .= '<a class="sf-button standard sf-icon-reveal wishlist-button" href="'.$yith_wcwl->get_wishlist_url().'"><i class="ss-star"></i><span class="text">'.__('Go to your wishlist', 'dante').'</span></a>';
	...
	/* AJAX SEARCH
	================================================== */
	if (!function_exists('dante_ajaxsearch')) {
		function dante_ajaxsearch() {
			...
			if (!empty($search_results)) {
				...
				foreach ($sorted_posts as $key => $type) {
			        ...
			        foreach ($type as $post) {
			            ...
			            $price = dante_get_post_meta( $post->ID, '_regular_price', true);
			            $sale = dante_get_post_meta( $post->ID, '_sale_price', true);
			            if ($sale != "") {
			           		$price = $sale;
			            }
			            if ($price != "") {
			            echo '<span>'.get_woocommerce_currency_symbol().$price.'</span>';
			            	}
			            } else {
			    ...
			    if ($count > 1) {
			    	echo '<a href="'.get_search_link($search_term).'" class="all-results">'.sprintf(__("View all %d results", 'dante'), $count).'</a>';
			    }

			} else {

				echo '<div class="no-search-results">';
				echo '<h6>'.__("No results", 'dante').'</h6>';
				echo '<p>'.__("No search results could be found, please try another query.", 'dante').'</p>';
				echo '</div>';
		...
		add_action('wp_ajax_dante_ajaxsearch', 'dante_ajaxsearch');
		add_action('wp_ajax_nopriv_dante_ajaxsearch', 'dante_ajaxsearch');
	}

	if (!function_exists('dante_ajaxurl')) {
		function dante_ajaxurl() {
		?>
		...
		add_action('wp_head','dante_ajaxurl');
	}
